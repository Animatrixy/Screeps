var action = new Creep.Action('guarding2');
action.reusePath = 10;
...
action.newTarget = function(creep){ 
    var flags = _.sortBy(_.filter(Game.flags, FLAG_COLOR.soldier.filter), 
        function(o) { 
...
action.work = function(creep){
    if( creep.room.situation.invasion ){
        if(creep.assignAction(creep.type == 'healer' ? Creep.action.healing : Creep.action.defending2))
